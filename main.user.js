...
const entrust = (() => {
  ...
  console.log("SMRC: use trusted type");
  let policy = trustedTypes.createPolicy("SMRCPolicy", {
    createHTML: s => s
  });
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  button.innerHTML = entrust(icon);
  button.title = "Send to Makereal Cafe!";
  button.hidden = true;
  document.body.append(button);
  button.addEventListener("click", sendCurPageToMRC);

  document.body.insertAdjacentHTML("beforeend", entrust`<style>
  ...
  let connectMRC = () => new Promise(res => {
    console.log("SMRC: connecting MRC")
  ...
  let networkChangeHandler = (whatEvent, { retryCnt = 20, maxTimeout = 5 * 60 * 1000 }) => async () => {
    console.log("SMRC: Network Change!");
  ...
  /*if(navigator.platform.match(/linux/i)) {
    console.log("SMRC: Linux")

    // "online/offline" events doesn't work on linux
    if(navigator.connection) { // chromium fallbacks to navigator.connection
      console.log("SMRC: Network Change = navigator.connection.onchange");
      navigator.connection.addEventListener("change", networkChangeHandler("networkChange"));
    } else { // firefox fallbacks to busy waiting
      console.log("SMRC: Network Change = busy waiting");
      while(true) {
        console.log("SMRC: busy waiting cycle in 5s.");
        await new Promise(res => setTimeout(res, 5000));
        if(session) continue;
        console.log("SMRC: MRC attempt.");
        await networkChangeHandler("online", { retryCnt: Infinity, maxTimeout: 5000 })();
      }
    }
  } else {
    console.log("SMRC: Network Change = window.ononline");
    window.addEventListener("online", networkChangeHandler("online"));
  }*/

  // according to Gemini (sorry I'm lazy), the most reliable way is still buzy waiting
  // - if there is some interface up (including virtual ones), the "online" will stay true
  // - SSID change in wifi may not cause navigator.connection.onchange event
  while(true) {
    console.log("SMRC: busy waiting cycle in 5s.");
    await new Promise(res => setTimeout(res, 5000));
    if(session) continue;
